...
//Create Admission
  ...
  await page1.getByPlaceholder('Type Name or Username').fill('Pavan');
  await page1.getByRole('button', { name: 'Refresh List' }).click();
  await page1.getByRole('link', { name: 'Automation , Pavan' }).click();
  await page1.waitForTimeout(2000);
  await expect(page1.getByTitle('Edit Owned User')).toBeVisible();
...
//Unassign facility to user in Edit owned page
  ...
  await page1.getByRole('link', { name: 'Automation , Pavan' }).click();
  await expect(page1.getByTitle('Edit Owned User')).toBeVisible();
  await expect(page1.getByText('Facility', { exact: true })).toBeVisible();
  await page1.locator('#linkEditFacilityTyypeAssignments').click();
  await page1.getByRole('cell', { name: 'Facility Name Starts With' }).getByRole('textbox').click();
  await page1.getByRole('cell', { name: 'Facility Name Starts With' }).getByRole('textbox').fill('Auto');
  await page1.getByRole('button', { name: 'Filter' }).click();
